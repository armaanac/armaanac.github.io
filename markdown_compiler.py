...
def compile_code_inline(line):
    ...
    for i in range (len(line)): 
        if line[i] == '`':
            if start_index == None:
                start_index= i
            elif start_index != None and stop_index == None:
                stop_index= i
            else:
                third_backtick = i

    if start_index is not None and stop_index is not None and third_backtick is None:
        tmp = ''
        for char in line[start_index + 1:stop_index]:
            if char == '<':
                tmp += '&lt;'
            elif char == '>':
                tmp += '&gt;'
                ...
                tmp += char
        new_line = line[:start_index] + '<code>' + tmp + '</code>' + line[stop_index+1:]
    else:
        new_line = line
...
def compile_links(line):
    ...
    if start_index is not None and stop_index is not None and start_index1 is not None and stop_index1 is not None:
        new_line = line[:start_index] + '<a ' + 'href=' +'"' + line[start_index1+1:stop_index1] + '"' + '>' + line[start_index+1:stop_index] + '</a>' + line[stop_index1+1:]
...
def compile_images(line):
    ...
    if start_index is not None and stop_index is not None and start_index1 is not None and stop_index1 is not None and e_index is not None:
        new_line = line[:start_index-1] + '<' +'img ' + 'src=' +'"' + line[start_index1+1:stop_index1] + '"' + ' alt=' + '"'+ line[start_index+1:stop_index] + '"' + ' />'+ line[stop_index1+1:]
